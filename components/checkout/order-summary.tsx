...
import { CheckCircle, CreditCard, Leaf, Truck, XCircle } from "lucide-react";
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-3 max-h-48 overflow-y-auto pr-2">
          {cartItems.map((item) => (
            ...
            >
              ...
              <span>{formatRupiah(item.discountedPrice * item.quantity)}</span>
            </div>
          ))}
        </div>
        <Separator className="bg-[#DE7C7D]/30 dark:bg-gray-700" />
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-between">
            <span className="text-gray-600 dark:text-gray-400">
              Biaya Pengiriman{" "}
              {deliveryOption === "delivery" && subtotal > 250000 && (
        ...
        <Button
          ...
          className="w-full bg-gradient-to-r from-[#AF1740] to-[#CC2B52] hover:from-[#740938] hover:to-[#AF1740] text-white rounded-2xl h-12 font-semibold shadow-lg hover:shadow-xl transition-all dark:from-[#FF4D6D] dark:to-[#CC2B52] dark:hover:from-[#CC2B52] dark:hover:to-[#FF4D6D]"
        >
          <CreditCard className="w-4 h-4 mr-2" />
          Selesaikan Pesanan Aman
        </Button>
        {/* Delivery Info */}
